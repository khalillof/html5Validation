...
function checkedUp(elm, extra=false) {
...
function checkfile(elm, filesNo = 2) {
...
function FormElements(form, extra = false, callback){
...
//============================= when form passed checks now second test inhouse checks
    ...
    let itemsValues=[] // store form values in case form will be returned to user, so values are used to populate the form

    FormElements(_form, true, (result, elm)=>{
     ...
     if(inHouseCheckFail) {
        ...
        let ee =_form.elements.namedItem(item.name);
        if(item.name === 'Terms'){
...
model.exports = {checkedUp, reg, checkfile, processChanges, processForm}
